...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Import Students', href: '/import/students' },
];
...
async function handleFileUpload() {
    ...
    try {
        const response = await fetch('/import/students/upload', {
            method: 'POST',
            headers: {
...
async function confirmImport() {
    ...
    try {
        const response = await fetch('/import/students/confirm', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({ students: validRows.value }),
        });
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-4 md:p-6">
            <div class="mb-6">
                <h1 class="text-2xl font-bold tracking-tight">Import Students</h1>
                <p class="text-muted-foreground">Upload, preview, and import student data from Excel.</p>
            </div>
            ...
            <Card v-if="step === 1">
                <CardHeader>
                    ...
                    <CardDescription>Select an .xlsx, .xls, or .csv file containing student data.</CardDescription>
                </CardHeader>
                <CardContent>
                            ...
                            <table class="w-full text-sm">
                                <thead class="sticky top-0 bg-background">
                                    <tr class="border-b">
                                        ...
                                        <th class="pb-2 pr-3 text-left font-medium">Last Name</th>
                                        <th class="pb-2 pr-3 text-left font-medium">First Name</th>
                                        <th class="pb-2 pr-3 text-left font-medium">Gender</th>
                                        <th class="pb-2 text-left font-medium">Birthdate</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="(row, i) in validRows" :key="i" class="border-b last:border-0">
                                        <td class="py-1.5 pr-3 font-mono text-xs">{{ row.lrn }}</td>
                                        <td class="py-1.5 pr-3">{{ row.last_name }}</td>
                                        <td class="py-1.5 pr-3">{{ row.first_name }}</td>
                                        <td class="py-1.5 pr-3 capitalize">{{ row.gender }}</td>
                                        <td class="py-1.5">{{ row.birthdate }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                            ...
                            <table class="w-full text-sm">
                                <thead class="sticky top-0 bg-background">
                                    <tr class="border-b">
                                        ...
                                        <th class="pb-2 pr-3 text-left font-medium">LRN</th>
                                        <th class="pb-2 pr-3 text-left font-medium">Name</th>
                                ...
                                <tbody>
                                    <tr v-for="(row, i) in invalidRows" :key="i" class="border-b last:border-0 bg-red-50 dark:bg-red-950/20">
                                        ...
                                        <td class="py-1.5 pr-3">{{ row.data.last_name }}, {{ row.data.first_name }}</td>
                                        <td class="py-1.5">
                ...
                <CardContent class="space-y-4">
                    <div class="flex items-center gap-3 text-green-600">
                        ...
                        <span class="text-lg font-medium">
                            {{ importResult?.imported ?? 0 }} students imported successfully
                        </span>
                    </div>
                    ...
                    <div v-if="importResult?.errors && importResult.errors.length > 0" class="mt-4">
                        ...
                        <ul class="list-inside list-disc text-sm text-red-600">
                            <li v-for="(err, i) in importResult.errors" :key="i">
                                Row {{ err.row }} (LRN: {{ err.lrn }}): {{ err.error }}
                            </li>
                        </ul>
                    </div>
